...
public class LocalMultiPbWallGridAdapter extends BaseAdapter implements StickyGridHeadersSimpleAdapter {
    ...
    public LocalMultiPbWallGridAdapter(Context context, List<LocalMediaItemInfo> list, FileType fileType) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            photoLayoutParams.height = (width - 3) / 4;
...
//            mViewHolder.mImageView.showBorder(true);
        ...
        if (file != null) {
            Glide.with(context).load(file).placeholder(R.drawable.pictures_no).into(mViewHolder.mImageView);
        }
        return convertView;
    }
    ...
    public void changeCheckBoxState(int position, OperationMode operationMode) {
        ...
        list.get(position).isItemChecked = !list.get(position).isItemChecked;
        this.notifyDataSetChanged();
    }

    public List<LocalMediaItemInfo> getCheckedItemsList() {
        LinkedList<LocalMediaItemInfo> checkedList = new LinkedList<LocalMediaItemInfo>();
