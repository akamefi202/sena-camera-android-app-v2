...
import com.sena.senacamera.function.USB.USBMonitor;
...
import com.sena.senacamera.data.AppInfo.ConfigureInfo;
import com.sena.senacamera.data.GlobalApp.GlobalInfo;
import com.sena.senacamera.data.Message.AppMessage;
import com.sena.senacamera.data.Mode.CameraNetworkMode;
import com.sena.senacamera.data.PropertyId.PropertyId;
import com.sena.senacamera.data.SystemInfo.HotSpot;
...
import com.sena.senacamera.data.entity.SearchedCameraInfo;
import com.sena.senacamera.data.entity.SelectedCameraInfo;
...
import com.sena.senacamera.ui.Interface.LaunchView;
...
import com.sena.senacamera.utils.PermissionTools;
...
import com.sena.senacamera.utils.imageloader.ICatchtekImageDownloader;
...
public class LaunchPresenter extends BasePresenter {
    ...
    public final LaunchHandler launchHandler = new LaunchHandler();
...
//        AppInfo.liveAddress = AppSharedPreferences.readDataByName( activity, AppSharedPreferences.OBJECT_NAME_LIVE_URL );
    ...
    public void addGlobalListener(int eventId, boolean forAllSession) {
        globalEvent.addGlobalEventListener(eventId, forAllSession);
    }

    public void delGlobalListener(int eventId, boolean forAllSession) {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                handler.post(new Runnable() {
                    @Override
                    public void run() {
                        beginConnectCamera(0, getCameraIp(), finalWifiSsid);
                    }
                });
    ...
    private void setReadyState(boolean isReady, String cameraName) {
        ...
        // akamefi202: make comment temporarily
...
//        }
    ...
    public void startSearchCamera() {
        ...
        addGlobalListener(ICatchCamEventID.ICATCH_EVENT_DEVICE_SCAN_ADD, false);
    ...
    public void startCameraConnecting() {
        launchHandler.obtainMessage(AppMessage.MESSAGE_CAMERA_CONNECTING_START, CameraType.WIFI_CAMERA, 0).sendToTarget();
    }
...
//                redirectToAnotherActivity(context, LaunchHelpActivity.class);
    ...
    public synchronized void connectCameraSena() {
        ...
        String finalWifiSsid = MWifiManager.getSsid(activity);
        MyProgressDialog.showProgressDialog(activity, R.string.action_processing);
...
//                redirectToAnotherActivity(activity, PreviewActivity.class);
    ...
    private void setCurrentCamera(int position, String ip, String wifiSsid) {
        ...
        activity.runOnUiThread(new Runnable() {
           ...
           public void run() {
               ...
               ((MainActivity) activity).compareFirmwareVersion();
           }
        });
    ...
    public void redirectToAnotherActivity(Context context) {
        ...
        if (cameraProperties != null
                && cameraProperties.hasFunction(PropertyId.DEFAULT_TO_PREVIEW)
    ...
    public synchronized void reconnectUSBCamera() {
        int position = (int) SharedPreferencesUtils.get(activity, SharedPreferencesUtils.CONFIG_FILE,"camera_position",0);
        UsbDevice usbDevice = getUsbDevice();
        beginConnectUSBCamera(position,usbDevice);
    }
            ...
            launchHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (Build.VERSION.SDK_INT >= 28 && !PermissionTools.checkCameraSelfPermission(activity)) {
                        SharedPreferencesUtils.put(activity, SharedPreferencesUtils.CONFIG_FILE,"camera_position", position);
